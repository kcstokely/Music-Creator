/***
	...
	W_TRACK_HEAD = FILE (WRITES ZERO FOR TRACK LENGTH)

    W_TRACK_FIX  = FILE / LOCATION / BYTES (FIXES ABOVE)
...
/***
  ...
  unsigned int  w_track_fix   (FILE, unsigned int, unsigned int);
  unsigned int 	w_file_head   (FILE, unsigned int, unsigned int, unsigned int);
...
unsigned int w_track_fix(FILE *fptr, unsigned int location, unsigned int bytes){
  
  unsigned int by[4];
  by[0]=bytes/(int)pow(2,24);
  by[1]=(bytes%(int)pow(2,24))/(int)pow(2,16);
  by[2]=(bytes%(int)pow(2,16))/(int)pow(2,8);
  by[3]=bytes%(int)pow(2,8);
...
unsigned int w_file_head(FILE *fptr, unsigned int synch, unsigned int tracks, unsigned int ppqn){
